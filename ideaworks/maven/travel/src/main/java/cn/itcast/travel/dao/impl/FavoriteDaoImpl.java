...
public class FavoriteDaoImpl implements FavoriteDao {
    ...
    public List<Route> findFavorite(int uid) {
        List RouteList = null;
        try {
            String sql = "SELECT * FROM tab_favorite f LEFT JOIN tab_route r ON f.rid = r.rid WHERE uid=? ORDER BY f.date DESC;";
    ...
    //收藏榜单
    @Override
    public List<Route> favoriteList() {
        List RouteList = null;
        try {
            String sql = "SELECT COUNT(1) COUNT, f.rid,r.rname,r.rimage,r.`price` FROM tab_favorite f JOIN tab_route r ON f.rid=r.rid GROUP BY f.rid ORDER BY COUNT DESC;";
